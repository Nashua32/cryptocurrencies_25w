...
def update_utxo_and_calculate_fee(tx, utxo):
    ...
    for inp in tx['inputs']:
        ...
        print(f"CHECKING FOR {in_txid} with INDEX {in_idx} IN UTXO")
        print(f"UTXO: {utxo}")
    ...
    print(f"Updated UTXO. New UTXO: {utxo}")

    return invalue - outvalue

# apply tx to mempool utxo
def update_mempool_utxo(tx, utxo):
    txid = get_objid(tx)
    ...
    for inp in tx['inputs']:
        ...
        print(f"CHECKING FOR {in_txid} with INDEX {in_idx} IN MEMPOOL UTXO")
    ...
    print(f"Updated Mempool UTXO. New Mempool UTXO: {utxo}")
...
def store_block(obj_dict, utxo, height, cur):
    ...
    print(f"Object with id {objid} is now in database")
