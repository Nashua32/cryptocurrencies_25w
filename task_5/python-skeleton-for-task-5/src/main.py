...
async def handle_object_msg(msg_dict, queue):
    ...
    try:
        ...
        if obj_dict['type'] == 'transaction':
            ...
            # we try to add the new transaction to the mempool
            MEMPOOL.try_add_tx(obj_dict)

        elif obj_dict['type'] == 'block':
            new_utxo, height = objects.verify_block(obj_dict)
            print(f"New UTXO: {new_utxo}")
            objects.store_block(obj_dict, new_utxo, height, cur)

            if height > CHAINTIP_HEIGHT:
                should_rebase_mempool = True
                old_chaintip = CHAINTIP
                #MEMPOOL.rebase_mempool(CHAINTIP, objid) # we have a new longest chain through either extending the longest chain or a chain reorg --> rebase mempool
        ...
        if should_rebase_mempool:
            print("Rebasing mempool....")
            MEMPOOL.rebase_mempool(old_chaintip, CHAINTIP)
            should_rebase_mempool = False
            old_chaintip = None
...
async def handle_mempool_msg(msg_dict):
    ...
    for txid in txids:
        tx = objects.get_object(txid)
        if tx is None:
            await broadcast_msg(mk_getobject_msg(txid)) # if tx is None, we send a getobject. The tx will then be added to mempool in handle_object_msg
        else:
...
async def handle_connection(reader, writer):
    ...
    try:
        ...
        await write_msg(writer, mk_getmempool_msg()) # according to the desc, we should send a getmempool msg immediately after hello
