...
def mk_peers_msg():
    self_host = getattr(const, "PUBLIC_HOSTNAME", LISTEN_CFG["address"])
    self_port = LISTEN_CFG["port"]

    self_peer = Peer(self_host, self_port)

    pl = [f'{peer}' for peer in PEERS if f'{peer}' != f'{self_peer}']
    if len(pl) > 30:
        pl = random.sample(pl, 30)

    pl.insert(0, f'{self_peer}')
